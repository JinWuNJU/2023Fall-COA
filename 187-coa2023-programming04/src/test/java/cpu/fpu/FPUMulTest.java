	...
	public void fpuMulTest9() {
		...
		for (int i = 0; i < input.length; i++) {
			for (int j = 0; j < input.length; j++) {
				src = new DataType(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(input[12]))));
				dest = new DataType(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(input[16]))));
				result = fpu.mul(src, dest);
				assertEquals(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(input[12] * input[16]))), result.toString());
			}
		}
	}
