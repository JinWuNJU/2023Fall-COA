...
import memory.cache.cacheMappingStrategy.AssociativeMapping;
import memory.cache.cacheMappingStrategy.DirectMapping;
import memory.cache.cacheMappingStrategy.MappingStrategy;
import memory.cache.cacheMappingStrategy.SetAssociativeMapping;
...
public class Cache {
    ...
    public void write(String pAddr, int len, byte[] data) {
        ...
        while (addr < upperBound) {
            ...
            if(!isWriteBack){
                ...
                memoryInstance.write(Transformer.intToBinary(Integer.toString(addr - addr % LINE_SIZE_B)),LINE_SIZE_B,cache_data);
                becomeValid(rowNO);
            }
            becomeDirty(rowNO);
    ...
    private int fetch(String pAddr) {
        ...
        if(map(blockNO) == -1){
            if(SETS == 512){
                ...
                return directMapping.writeCache(blockNO);
            }else if(SETS == 1){
                AssociativeMapping associativeMapping = new AssociativeMapping();
                return this.mappingStrategy.writeCache(blockNO);
            }else {
    ...
    private int map(int blockNO) {
        if(SETS == 512){
            DirectMapping directMapping = new DirectMapping();
            return directMapping.map(blockNO);
        }else if(SETS == 1){
            //AssociativeMapping associativeMapping = new AssociativeMapping();
            return this.mappingStrategy.map(blockNO);
        }else {
            //SetAssociativeMapping setAssociativeMapping = new SetAssociativeMapping();
            return this.mappingStrategy.map(blockNO);
        }
    ...
    public void setSETS(int SETS) {
        this.SETS = SETS;
        if(SETS == 1){
            this.mappingStrategy = new AssociativeMapping();
        }else if(SETS == 512){
    ...
    public void becomeValid(int rowNO){
        this.cache[rowNO].validBit = true;
    }
    ...
    public void becomeDirty(int rowNO){
        this.cache[rowNO].dirty = true;
    }
    ...
    public void setCacheVisited(int lineN0,int visited){
        this.cache[lineN0].visited = visited;
    }
    ...
    public void setTimeStamp_me(int rowNo,long timestamp){
        this.cache[rowNo].timeStamp = timestamp;
    }
    ...
    public void setTimeStampFIFO(int rowNo){
        ...
        if((rowNo+1)%setSize == 0){
            cache[rowNo+1-setSize].timeStamp = 2L;
        }else{
            cache[rowNo+1].timeStamp = 2L;
        }
    }

    public void setCacheLine(int lineN0, char[] tag, byte[] data) {
        this.cache[lineN0].tag = tag;
        this.cache[lineN0].data = data;
        this.cache[lineN0].validBit = true;
    }
    ...
    public char[] getTag(int rowNO){return cache[rowNO].getTag();}
